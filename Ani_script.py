...
def V_analysis(SIM=False, i_V=False):
   ...
   #Wong Analytical formula for Fusion Cross section
    ...
    if Show_potential == True:
        ...
        plt.legend()
        plt.grid()
        plt.savefig('Potentials_High.png',dpi=400,bbox_inches='tight',)
        #plt.show()
    ...
    if Sim_Analysis == True:
        ...
        #sigma_scan = np.array([1, 2.5, 5, 7.5, 10])
        ...
        #ax.title.set_text("Transmission probability")
        ax.semilogy(E, T0_E, label='Analytical  - Hill Wheeler',color='blue',ls='-')
        ax.semilogy(E_num, T0_num, ls='dotted',color='red', label='Numerical - TISE')
        for X in range (n_sigma):
            ax.semilogy(Energy[X,:],T_P[X,:], label=f'$\sigma={sigma_scan[X]}$ fm', marker='x',ls='--',markersize=15)
        ...
        plt.legend()
        plt.grid()
        plt.savefig('Tran_prob_High.png',dpi=400,bbox_inches='tight',)
        #plt.show()
...
def Main():
    ...
    for k in range(n):
        ...
        if FILM_Psi_real == True:
            ...
            anim.save(f'Psi_E{E0}.mp4',  writer = FFwriter,dpi=80)
